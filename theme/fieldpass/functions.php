 ...
 */
function fieldpass_enqueue_styles() {
	$theme = wp_get_theme();
	wp_enqueue_style( 'fieldpass', get_stylesheet_uri(), array(), $theme->get( 'Version' ) );
}
add_action( 'wp_enqueue_scripts', 'fieldpass_enqueue_styles' );

function fieldpass_setup() {
	add_editor_style( 'style.css' );

	// Lets editors write a short summary for pages. Used for the meta description.
	add_post_type_support( 'page', 'excerpt' );
}
add_action( 'after_setup_theme', 'fieldpass_setup' );

/**
 * Group the theme's patterns under one "Fieldpass" heading in the inserter.
 */
function fieldpass_pattern_categories() {
	register_block_pattern_category( 'fieldpass', array( 'label' => __( 'Fieldpass sections', 'fieldpass' ) ) );
	register_block_pattern_category( 'fieldpass-pages', array( 'label' => __( 'Fieldpass page layouts', 'fieldpass' ) ) );
}
add_action( 'init', 'fieldpass_pattern_categories' );
...
function fieldpass_load_form_assets() {
...
add_action( 'wp_enqueue_scripts', 'fieldpass_load_form_assets', 20 );
 ...
 */
function fieldpass_meta_description() {
...
add_action( 'wp_head', 'fieldpass_meta_description', 2 );
