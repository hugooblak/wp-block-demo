 ...
 */
function fieldpass_faq_schema_items( $item = null ) {
	static $items = array();
	if ( is_array( $item ) ) {
...
function fieldpass_faq_print_schema() {
	$items = fieldpass_faq_schema_items();
	if ( empty( $items ) ) {
		return;
	}
...
add_action( 'wp_footer', 'fieldpass_faq_print_schema' );
